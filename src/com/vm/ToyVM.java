...
import com.toy.ast.NativeFunction;
...
public class ToyVM {
	...
	public ToyVM(int codeSize,int stackSize,int stringSize,HeapMemory hm){
	...
	public void mainLoop(){
		switch (code[pc]) {
		case ICONST:
			registers[code[pc + 5]] = readInt(code, pc+1);
			pc+=6;
			break;
		case BCONST:
		...
		case NEG:{
			...
			if(v instanceof Integer){
				...
				throw new ToyException("bad operand value");
			}
			pc += 2;
		}
			break;
		default:
			if(code[pc] > LESS){
				throw new ToyException("bad instruction");
			}else{
				computeNumber();
			}
			break;
	...
	protected void computeNumber(){
		...
		if(code[pc] == ADD && !areNumbers){
			...
			case LESS:
				...
				break;
			default:
				throw new ToyException("never reach here");
			}
			registers[left] = i3;
		}
		pc += 3;
