...
public class Opcode {
	...
	public static final byte BCONST = 2; //load an 8bit integer
	public static final byte SCONST = 3; //load a character string
	public static final byte MOVE = 4; //move a value
	public static final byte GMOVE = 5; //move a value(gloabal varible)
	...
	public static final byte SUB = 14; //substract
	...
	/*
	 * ±àÂë¼Ä´æÆ÷,µÚi¸ö¼Ä´æÆ÷Î»-reg-1;
	 * */
	public static byte encodeRegister(int reg){
		if(reg > ToyVM.NUM_OF_REG){
			throw new ToyException("too many registers required");
		}else{
			return (byte)-(reg + 1);
		}
	}
	...
	public static byte encodeOffset(int offset){
		if(offset > Byte.MAX_VALUE){
			throw new ToyException("too big byte offset");
		}else{
			return (byte)offset;
		}
	}
	public static short encodeShortOffset(int offset){
		if(offset < Short.MIN_VALUE || Short.MAX_VALUE < offset){
			throw new ToyException("too big short offset");
		}else{
			return (short)offset;
		}
	}
